...
const CardEditor = () => {
  ...
  const handleVCardImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!looksLikeVCard(file.name, text)) {
      toast({ title: "Not a vCard file", description: "Please upload a .vcf contact file.", variant: "destructive" });
      e.target.value = "";
      return;
    }
    ...
    if (fieldsFound === 0) {
      toast({ title: "Nothing to import", description: "That vCard didn't contain any recognizable fields.", variant: "destructive" });
      e.target.value = "";
      return;
    }
    setCard((c) => ({ ...c, ...parsed }));
    toast({ title: "Imported from vCard", description: `Filled in ${fieldsFound} field${fieldsFound === 1 ? "" : "s"}${parsed.full_name ? ` from ${parsed.full_name}'s contact` : ""}.` });
    e.target.value = "";
  };
                ...
                <>
                  <div className="space-y-3 rounded-xl border border-dashed border-border bg-secondary/40 p-4">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <h3 className="text-sm font-bold">Import from vCard</h3>
                        <p className="text-xs text-muted-foreground mt-0.5">Upload a .vcf file to auto-fill your info below.</p>
                      </div>
                      <label className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full bg-secondary border border-border text-xs font-medium cursor-pointer hover:border-primary/50 transition-colors">
                        <FileUp size={13} />Choose file
                        <input type="file" accept=".vcf,text/vcard,text/x-vcard" onChange={handleVCardImport} className="hidden" />
                      </label>
                    </div>
                  </div>
                  <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <OptionGroup
                      options={[
                        ...
                        { value: "buttons", label: "Labeled Buttons" },
                        { value: "compact", label: "Compact List" },
                      ]}
                      value={card.social_display_style || "icons"}
                      onChange={(v) => update("social_display_style", v)}
                    />
                  </div>
